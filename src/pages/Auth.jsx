...
export default function Auth() {
  ...
  // Initial mount: wait for turnstile (preloaded in index.html)
  useEffect(() => {
  ...
  const toggleTheme = () => {
    ...
    // Re-render widget with new theme
  ...
  const handleAuth = async (e) => {
    e.preventDefault()
    if (!token) return setMsg({ type: 'error', text: t.captchaRequired })
    if (mode === 'register' && !terms) return setMsg({ type: 'error', text: t.termsRequired })
    setLoading(true); setMsg({ type: '', text: '' })
    const fn = mode === 'login' ? signIn : signUp
    const { error } = await fn(email, password, token)
    if (error) {
    ...
    // On login success, AppContext updates user → App.jsx redirects automatically
    setLoading(false)
  }
      ...
      <div className="auth-card card">
        ...
        <form onSubmit={handleReset}>
          ...
          <div className="captcha-wrap">
            ...
            {!ready && <p className="captcha-hint">{t.captchaLoading}</p>}
          </div>
        ...
        <form onSubmit={handleAuth}>
          ...
          {mode === 'login' && (
            <div className="forgot-row">
              <button type="button" className="link-btn-sm"
                onClick={() => goTo('reset')}>
                {t.forgotPassword}
              </button>
            </div>
